...
namespace CherryCouch.Common.Config
{
    public class ConfigVariable : ConfigNode
    {
        ...
        internal override void Save(XmlWriter writer, object instance)
        {
        ...
        internal override void Load(XmlNode node, object instance)
        {
            SetValue(new XmlSerializer(VariableType).Deserialize(new StringReader(node.InnerXml)), instance);
            IsSynchronised = true;
        }
    }
}